...
pub fn ensure_redis_web_debug_binaries() {
...
fn resolve_binary_path(name: &str) -> PathBuf {
    let direct_key = format!("CARGO_BIN_EXE_{name}");
    if let Ok(path) = std::env::var(&direct_key) {
        return PathBuf::from(path);
    }

    let underscored = name.replace('-', "_");
    let alt_key = format!("CARGO_BIN_EXE_{underscored}");
...
pub fn redis_web_binary_path() -> PathBuf {
    resolve_binary_path("redis-web")
}

pub fn webdis_binary_path() -> PathBuf {
...
pub async fn raw_http_get(port: u16, request_target: &str) -> (u16, String) {
    let mut stream = TcpStream::connect(("127.0.0.1", port))
        .await
        .expect("failed to connect to redis-web test server");
...
impl TestServer {
    ...
    pub async fn spawn_with_config_and_env(
        ...
        for attempt in 0..20 {
            ...
            let _ = process.kill();
            if attempt == 19 {
                panic!("failed to start redis-web after retries");
            }
        }
