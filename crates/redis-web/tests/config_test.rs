...
#[test]
fn test_default_document_can_use_redis_web_schema_reference() {
    let value = Config::default_document("./redis-web.schema.json");
    let obj = value
        .as_object()
        .expect("default document should be a JSON object");
    assert_eq!(
...
#[test]
fn test_legacy_webdis_fixture_still_loads() {
    ...
    assert!(
        config.is_ok(),
        "webdis.legacy.json should remain loadable for compatibility: {config:?}"
    );
}
