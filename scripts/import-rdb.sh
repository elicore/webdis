...
# Import a local dump.rdb into the docker volume used by docker/docker-compose.rdb.yml
...
echo "Starting stack with docker/docker-compose.rdb.yml"
docker compose -f docker/docker-compose.rdb.yml up --build
